...
import { transformClinic, transformClinicToBackend } from '../utils/dataTransform';
...
const ClinicManager: React.FC<ClinicManagerProps> = ({ clinics, setClinics, onRefresh }) => {
  ...
  const handleSave = async () => {
    ...
    // Ensure numeric values have defaults if left empty
    ...
    try {
      const clinicData = transformClinicToBackend({
        ...editingClinic,
        consultationFee,
        maxPatientsPerDay,
        currentServingNumber: editingClinic.currentServingNumber || 0,
        isClosedToday: editingClinic.isClosedToday || false,
      } as Clinic);

      if (editingClinic.id) {
        await clinicAPI.update(editingClinic.id, clinicData);
        showToast('تم تحديث العيادة بنجاح', 'success');
      } else {
        await clinicAPI.create(clinicData);
  ...
  // Helper to handle numeric input changes without the "Zero Issue"
  ...
  return (
    <div className="space-y-8 text-right">
      <div className="flex justify-between items-center">
        ...
        <button 
          onClick={() => { setEditingClinic({ consultationFee: 300, maxPatientsPerDay: 20 }); setShowModal(true); }}
        ...
        {clinics.map(clinic => (
          <div key={clinic.id} className="bg-white overflow-hidden rounded-3xl border border-slate-100 shadow-sm hover:border-blue-200 transition-all group flex flex-col">
            <div className="h-40 relative">
               <img src={clinic.photo || 'https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?auto=format&fit=crop&q=80&w=400&h=200'} className="w-full h-full object-cover" alt={clinic.name} />
               ...
               <div className="absolute top-4 left-4 flex gap-1">
                 <button onClick={() => { setEditingClinic(clinic); setShowModal(true); }} className="p-2 bg-white/20 backdrop-blur-md rounded-lg text-white hover:bg-white/40 transition-all"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                 <button onClick={() => deleteClinic(clinic.id)} className="p-2 bg-rose-500/20 backdrop-blur-md rounded-lg text-white hover:bg-rose-500/40 transition-all"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
               </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] w-full max-w-2xl p-10 shadow-2xl border border-slate-100 text-right overflow-y-auto max-h-[90vh] animate-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="space-y-6">
                  <div>
                    ...
                    <div className="relative h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center group overflow-hidden transition-colors hover:border-blue-400">
                       {editingClinic.photo ? (
                         <img src={editingClinic.photo} className="w-full h-full object-cover" />
                       ) : (
                         <>
               ...
               <div className="space-y-6">
                  <div>
                    ...
                    <input type="text" className="w-full px-4 py-3 rounded-2xl border border-slate-200 text-right outline-none focus:ring-4 focus:ring-blue-500/10 font-medium transition-all" value={editingClinic.name || ''} onChange={e => setEditingClinic({...editingClinic, name: e.target.value})} placeholder="مثال: فرع المهندسين" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">العنوان</label>
                    <input type="text" className="w-full px-4 py-3 rounded-2xl border border-slate-200 text-right outline-none focus:ring-4 focus:ring-blue-500/10 font-medium transition-all" value={editingClinic.address || ''} onChange={e => setEditingClinic({...editingClinic, address: e.target.value})} placeholder="العنوان بالتفصيل" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <input type="text" className="w-full px-4 py-3 rounded-2xl border border-slate-200 text-right outline-none focus:ring-4 focus:ring-blue-500/10 font-medium transition-all" value={editingClinic.workingHours || ''} onChange={e => setEditingClinic({...editingClinic, workingHours: e.target.value})} placeholder="مثال: 4م - 10م" />
                    </div>
                  </div>
                  <div>
            ...
            <div className="flex gap-4 mt-10">
              <button onClick={() => setShowModal(false)} className="flex-1 py-4 text-xs font-bold text-slate-400 bg-slate-100 rounded-2xl hover:bg-slate-200 transition-all">إلغاء</button>
              <button onClick={handleSave} className="flex-1 py-4 text-xs font-bold text-white bg-blue-600 rounded-2xl shadow-xl shadow-blue-500/30 hover:bg-blue-700 transition-all">تأكيد البيانات</button>
            </div>
          </div>
        </div>
      )}
