...
const Orders: React.FC<OrdersProps> = ({ orders, setOrders, selectedClinicId, clinics, setClinics, doctorName, timeSlots, setTimeSlots, onRefresh }) => {
 ...
 const filteredOrders = orders.filter(o => {
  ...
  if (dateFilter === 'اليوم') matchesDate = o.date === todayStr;
 ...
 // Logic Point 1 & 4: Restore availability and ensure data integrity upon deletion
  ...
  <div className="space-y-6 text-right relative pb-10 animate-slide-in">
   ...
   <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
    <div>
     ...
     <p className="text-slate-500 text-sm mt-1">تتبع تدفق المرضى لفرع: <span className="text-blue-600 font-bold">{activeClinic.name}</span></p>
    </div>
    ...
    <div className="flex flex-wrap lg:flex-nowrap gap-3 w-full lg:w-auto items-center">
     ...
     <button onClick={handleCallNext} className="flex-1 lg:flex-none px-8 py-4 bg-emerald-600 text-white rounded-2xl hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 transition-all font-bold text-xs">
      استدعاء التالي
     </button>
     <button onClick={() => { setCurrentOrder({ phoneNumber: '01', status: 'منتظر' }); setShowModal(true); }} className="flex-1 lg:flex-none px-8 py-4 bg-slate-900 text-white rounded-2xl hover:bg-slate-800 shadow-lg transition-all font-bold text-xs">
      + حجز مريض جديد
     </button>
    </div>
   </div>
   ...
   <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-sm">
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 md:gap-6 items-end">
     ...
     <div className="w-full">
      ...
      <select className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-xs font-bold cursor-pointer hover:bg-slate-100 transition-colors" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)}>
       <option value="الكل">كل الأوقات</option>
       <option value="اليوم">حجوزات اليوم</option>
      </select>
     </div>

     <div className="w-full">
      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 mr-2">حالة الكشف</label>
      <select className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-xs font-bold cursor-pointer hover:bg-slate-100 transition-colors" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
     ...
     <div className="w-full">
      <button onClick={resetFilters} className="w-full h-[58px] px-6 text-blue-600 bg-blue-50 border border-blue-100 rounded-2xl font-black text-xs hover:bg-blue-100 transition-all flex items-center justify-center gap-3">
      ...
      <tbody className="divide-y divide-slate-50">
       {filteredOrders.length > 0 ? filteredOrders.map((order) => (
        <tr key={order.id} className="hover:bg-slate-50/50 transition-all group">
         ...
         <td className="px-6 py-6">
          <span className={`px-3 py-1 rounded-xl text-[10px] font-bold border ${order.type === 'كشف' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' :
         ...
         <td className="px-6 py-6 text-center">
          <a href={getWhatsAppLink(order)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-10 h-10 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white hover:scale-110 transition-all">
         ...
         <td className="px-8 py-6 text-left">
          <div className="flex gap-2 justify-start lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
           <button onClick={() => { setCurrentOrder(order); setShowModal(true); }} className="p-2 text-slate-400 hover:text-blue-600 transition-all bg-white border border-slate-100 rounded-xl hover:shadow-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
           </button>
           <button onClick={() => { setOrderToDeleteId(order.id); setShowDeleteConfirm(true); }} className="p-2 text-slate-400 hover:text-rose-600 transition-all bg-white border border-slate-100 rounded-xl hover:shadow-sm">
   ...
   {/* Delete Confirmation Modal (Satisfies Real-time UI and Data Integrity Prompt) */}
   {showDeleteConfirm && (
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[250] p-4 animate-in fade-in duration-300">
     <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl border border-slate-100 text-center animate-in zoom-in-95 duration-300">
      ...
      <p className="text-sm text-slate-500 mb-8 leading-relaxed">عند حذف هذا المريض، سيتم استعادة مكان شاغر في جدول المواعيد تلقائياً وتعديل الإحصائيات المالية.</p>
      <div className="flex gap-4">
       <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-4 text-xs font-bold text-slate-400 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-all">إلغاء</button>
       <button onClick={confirmDelete} className="flex-1 py-4 text-xs font-bold text-white bg-rose-600 rounded-2xl shadow-xl shadow-rose-600/20 hover:bg-rose-700 transition-all">نعم، حذف الحجز</button>
      </div>
     </div>
    </div>
   )}
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4 animate-in fade-in duration-300">
     <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-8 md:p-12 shadow-2xl border border-slate-100 text-right overflow-y-auto max-h-[90vh] animate-in zoom-in-95 duration-300">
      ...
      <div className="space-y-6">
       ...
       <div className="space-y-2">
        ...
        >
         ...
         {timeSlots.filter(ts => ts.clinicId === selectedClinicId && ts.bookedCount < ts.capacity).map(ts => (
          <option key={ts.id} value={ts.id}>{ts.day} - {ts.startTime} ({ts.bookedCount}/{ts.capacity})</option>
         ))}
        </select>
       </div>
      </div>
      ...
      <div className="flex gap-6 mt-12 pt-8 border-t border-slate-50">
       <button onClick={() => setShowModal(false)} className="flex-1 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-all">إلغاء الأمر</button>
       <button onClick={handleSave} className="flex-1 py-5 text-[11px] font-black uppercase tracking-widest text-white bg-blue-600 rounded-2xl shadow-2xl shadow-blue-500/20 hover:bg-blue-700 transition-all">
